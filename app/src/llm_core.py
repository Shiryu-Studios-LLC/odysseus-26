...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt Shirabi' canonical OpenAI-style messages to native Ollama /api/chat.

    Shirabi carries assistant tool calls in the OpenAI shape, where
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/Shiryu-Studios-LLC/shirabi26")
        h.setdefault("X-OpenRouter-Title", "Shirabi")
    if provider == "copilot":
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip Shirabi-only metadata before sending messages to providers.
