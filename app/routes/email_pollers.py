                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Shirabi (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Shirabi" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "shirabi_kind" if "shirabi_kind" in cols else "'scheduled' AS shirabi_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                shirabi_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Shirabi-Origin"] = "shirabi-ui"
                outer["X-Shirabi-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", shirabi_kind or "scheduled")[:64]
                outer["X-Shirabi-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `shirabi-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `SHIRABI_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `shirabi-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("SHIRABI_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `SHIRABI_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (SHIRABI_INPROCESS_POLLERS=0); "
            "drive `shirabi-mail poll-scheduled` externally."
        )
        return
    import asyncio
